...
class AIAssistantManager:
    """AI assistant for cybersecurity analysis"""
    ...
    def __init__(self):
        try:
            ...
            When analyzing alerts, provide clear explanations in this structure:
            ...
            4. **Recommended Actions**: Specific, prioritized remediation steps
    ...
    def _format_markdown_to_plain(self, text):
        ...
        # Replace numbered lists (make them more readable)
        text = re.sub(r'^(\d+)\.\s+', r'\n\1. ', text, flags=re.MULTILINE)
    ...
    def analyze_alert(self, alert_data):
        """Analyze a specific alert and provide detailed explanation"""
        ...
        try:
            ...
            context = f"""
Analyze this security alert from FortifAI:
...
"""
    ...
    def chat(self, user_message, context_data=None):
        """General chat interface with optional context"""
        if not self.available:
            return "AI Assistant is not available. Please check your API key."
    ...
    def get_remediation_steps(self, alert_type, severity):
        """Get specific remediation steps for alert type"""
        if not self.available:
            return []
        
        try:
            prompt = f"""
As a cybersecurity expert, provide a numbered list of specific remediation steps for this scenario:
- Alert Type: {alert_type}
- Severity: {severity}

Format as a simple numbered list (1., 2., 3., etc.) with actionable steps.
Maximum 7 steps, prioritized by urgency.
"""
            response = self.model.generate_content(prompt)
            formatted_response = self._format_markdown_to_plain(response.text)
            # Parse numbered list
            lines = formatted_response.text.strip().split('\n')
            steps = []
            for line in lines:
                line = line.strip()
...
class FederatedLearningManager:
    ...
    def aggregate_models(self):
        """
        FIXED: True federated learning with ACTUAL weight updates and convergence
        """
        ...
        try:
            ...
            # ========================================================================
            # FIX: PROPER FEDERATED AVERAGING WITH MOMENTUM
            # ========================================================================
            aggregated_weights = {}
            ...
            # LEARNING RATE: Controls how much to move toward aggregated values
            LEARNING_RATE = 0.5  # Increased from 0.3 for faster convergence
            MOMENTUM = 0.9
            ...
            for key in param_keys:
                ...
                if total_weight > 0:
                    ...
                    # ✅ FIX: Apply momentum for smoother convergence
                    momentum_key = key
                    if momentum_key in self.global_model['momentum']:
                        momentum_term = self.global_model['momentum'][momentum_key]
                    else:
                        momentum_term = 0.0
                    
                    # Compute gradient
                    gradient = aggregated_value - old_value
                    
                    # Update momentum
                    new_momentum = MOMENTUM * momentum_term + (1 - MOMENTUM) * gradient
                    self.global_model['momentum'][momentum_key] = new_momentum
                    
                    # Apply update with momentum
                    new_value = old_value + LEARNING_RATE * new_momentum
                    
                    # ✅ Clamp to reasonable bounds
                    if 'threshold' in key:
                        new_value = np.clip(new_value, 1.0, 5.0)
                    elif 'sensitivity' in key:
                        new_value = np.clip(new_value, 0.3, 2.0)
                    ...
                    if abs(new_value - old_value) > 0.01:
                        print(f"  ✓ {key}: {old_value:.4f} → {new_value:.4f} (Δ={gradient:.4f})")
                else:
            ...
            # ========================================================================
            aggregated_baselines = self._aggregate_baselines_robust(self.pending_updates)
            ...
            if should_rollback:
                ...
                self.global_model['momentum'] = {k: 0.0 for k in self.global_model['momentum'].keys()}
...
class DatabaseManager:
    ...
    def analyze_and_alert(self, client_id, telemetry_data):
        ...
        # ✅ FIX: Add server-side alert cache with TTL
...
class MainWindow(QMainWindow):
    ...
    def on_alert_selected(self):
        """Handle alert selection - show preview in AI panel"""
        selected = self.alerts_table.currentRow()
        if selected < 0:
            return
        
        # Get alert data
        timestamp = self.alerts_table.item(selected, 0).text()
        severity = self.alerts_table.item(selected, 1).text()
        client = self.alerts_table.item(selected, 2).text()
        category = self.alerts_table.item(selected, 3).text()
        title = self.alerts_table.item(selected, 4).text()
        description = self.alerts_table.item(selected, 5).text()
        risk = self.alerts_table.item(selected, 6).text()
    ...
    Click "🔍 Analyze Selected" for AI-powered deep analysis,
    or ask questions in the chat below.
    """
        
        self.alert_analysis_text.setText(preview)

    def format_ai_response_for_display(self, text):
        """
        Additional formatting for display in Qt widgets
        Adds proper spacing and visual separators
        """
        lines = text.split('\n')
        formatted_lines = []
        ...
        for line in lines:
            ...
            if stripped.startswith('━━━') or stripped.startswith('═══'):
                ...
                formatted_lines.append('')
            # Preserve bullet points with proper indentation
    ...
    def analyze_selected_alert(self):
        """Analyze selected alert with AI - ENHANCED FORMATTING"""
        selected = self.alerts_table.currentRow()
        if selected < 0:
            QMessageBox.warning(self, "No Selection", "Please select an alert to analyze")
            return
        
        # Show loading
        self.alert_analysis_text.setText("🔄 Analyzing alert with AI...\n\nPlease wait...")
        QApplication.processEvents()
        
        # Get full alert data from database
        cursor = self.db_manager.connection.cursor(cursor_factory=RealDictCursor)
        
        # Build query with filters
        where_clauses = self._build_alert_filter_clauses()
        ...
        if not alert:
            self.alert_analysis_text.setText("❌ Error: Could not retrieve alert data")
            return
        
        # Get AI analysis
        analysis = self.ai_assistant.analyze_alert(alert)
        
        # Apply additional display formatting
        formatted_analysis = self.format_ai_response_for_display(analysis)
    ...
    """
        ...
        self.alert_analysis_text.setText(result)

    def send_ai_chat(self):
        """Send message to AI assistant - ENHANCED FORMATTING"""
        message = self.ai_chat_input.text().strip()
        if not message:
            return
        
        # Get context from selected alert if any
        context = None
        selected = self.alerts_table.currentRow()
        if selected >= 0:
            title = self.alerts_table.item(selected, 4).text()
            description = self.alerts_table.item(selected, 5).text()
            context = f"Current Alert Context:\nTitle: {title}\nDescription: {description}"
        
        # Show user message
        current_text = self.alert_analysis_text.toPlainText()
        self.alert_analysis_text.setText(
        ...
        # Apply additional display formatting
        formatted_response = self.format_ai_response_for_display(response)
        
        # Update display
        current_text = self.alert_analysis_text.toPlainText()
        current_text = current_text.replace("🤖 AI Assistant: Thinking...", formatted_response)
        self.alert_analysis_text.setText(current_text)
        
        # Scroll to bottom
        scrollbar = self.alert_analysis_text.verticalScrollBar()
    ...
    def export_alerts(self):
        ...
        filename, _ = QFileDialog.getSaveFileName(
            self, "Export Alerts", "", "CSV Files (*.csv);;All Files (*)"
        )
        ...
        try:
            ...
            QMessageBox.information(
                ...
                f"Exported {self.alerts_table.rowCount()} alerts to:\n{filename}"
            )
        
        except Exception as e:
            QMessageBox.critical(self, "Export Failed", f"Error: {str(e)}")

    def _build_alert_filter_clauses(self):
        """Build SQL WHERE clauses from filter selections"""
        where_clauses = ["a.status = 'active'"]
        
        # Category filter
        category = self.alert_category_filter.currentText()
        if category != "All Categories":
            ...
            where_clauses.append(f"a.source_category = '{category_map[category]}'")
        
        # Severity filter
        severity = self.alert_severity_filter.currentText()
        if severity != "All Severities":
            where_clauses.append(f"a.severity = '{severity.lower()}'")
        
        # Time filter
        time_filter = self.alert_time_filter.currentText()
        time_map = {
            "Last Hour": "1 hour",
        ...
        if time_filter in time_map:
            where_clauses.append(f"a.timestamp > NOW() - INTERVAL '{time_map[time_filter]}'")
        
        # Client filter
        selected_client = self.alert_client_filter.currentData()
        if selected_client:
            where_clauses.append(f"a.client_id = '{selected_client}'")
    ...
    def create_alerts_tab(self):
        """Create enhanced alerts management tab with filtering and AI analysis"""
        alerts = QWidget()
        main_layout = QHBoxLayout(alerts)  # Changed to horizontal for split view
        ...
        self.alert_category_filter.currentIndexChanged.connect(self.load_alerts)
        row1_layout.addWidget(self.alert_category_filter)
        
        row1_layout.addWidget(QLabel("Severity:"))
        self.alert_severity_filter = QComboBox()
        self.alert_severity_filter.addItems([
            "All Severities",
            "Critical",
            "High",
            "Medium",
        ...
        self.alert_severity_filter.currentIndexChanged.connect(self.load_alerts)
        row1_layout.addWidget(self.alert_severity_filter)
        
        row1_layout.addStretch()
        filter_layout.addLayout(row1_layout)
        
        # Row 2: Time range and client filters
        row2_layout = QHBoxLayout()
        
        row2_layout.addWidget(QLabel("Time:"))
        self.alert_time_filter = QComboBox()
        self.alert_time_filter.addItems([
            "Last Hour",
            "Last 24 Hours",
            "Last 7 Days",
            "Last 30 Days",
        ...
        self.alert_time_filter.setCurrentIndex(1)  # Default: Last 24 Hours
        self.alert_time_filter.currentIndexChanged.connect(self.load_alerts)
        row2_layout.addWidget(self.alert_time_filter)
        
        row2_layout.addWidget(QLabel("Client:"))
        self.alert_client_filter = QComboBox()
        self.alert_client_filter.addItem("All Clients", None)
        self.alert_client_filter.currentIndexChanged.connect(self.load_alerts)
        row2_layout.addWidget(self.alert_client_filter)
        ...
        clear_filters_btn.clicked.connect(self.clear_alert_filters)
        btn_layout.addWidget(clear_filters_btn)
        ...
        left_layout.addWidget(self.alerts_table)
        
        main_layout.addWidget(left_panel, stretch=6)
        
        # ============================================================================
        # NEW: Right panel - AI Analysis
        # ============================================================================
        right_panel = QWidget()
        right_layout = QVBoxLayout(right_panel)
        
        ai_group = QGroupBox("🤖 AI Security Assistant")
        ai_layout = QVBoxLayout()
        
        # Alert analysis display
        self.alert_analysis_text = QTextEdit()
        self.alert_analysis_text.setReadOnly(True)
        self.alert_analysis_text.setPlaceholderText(
            "Select an alert from the table to see AI-powered analysis...\n\n"
            "Or use the chat below to ask security questions."
        )
        ai_layout.addWidget(self.alert_analysis_text)
        
        # Chat interface
        chat_layout = QHBoxLayout()
        self.ai_chat_input = QLineEdit()
        self.ai_chat_input.setPlaceholderText("Ask the AI assistant anything about security...")
        self.ai_chat_input.returnPressed.connect(self.send_ai_chat)
        ...
        chat_layout.addWidget(analyze_btn)
    ...
    def refresh_client_filters(self):
        """Populate client filter dropdowns - ENHANCED"""
        clients = self.db_manager.get_all_clients()
        
        for combo in [self.network_client_filter, self.process_client_filter, 
                    self.fs_client_filter, self.user_client_filter,
                    self.alert_client_filter]:  # ✅ ADD THIS
            combo.clear()
    ...
    def load_alerts(self):
        """Load alerts with filtering - ENHANCED"""
        # Disconnect signals temporarily
        try:
            self.alert_category_filter.currentIndexChanged.disconnect()
            self.alert_severity_filter.currentIndexChanged.disconnect()
            self.alert_time_filter.currentIndexChanged.disconnect()
            self.alert_client_filter.currentIndexChanged.disconnect()
        except:
            pass
        ...
        # Update status
        self.statusBar().showMessage(f"Loaded {len(alerts)} alerts")
