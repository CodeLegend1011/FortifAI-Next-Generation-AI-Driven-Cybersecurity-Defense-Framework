...
class FederatedLearningManager:
    ...
    def aggregate_models(self):
        """
        Advanced aggregation with Trimmed Mean, Reputation Weighting, and DP
        """
        ...
        try:
            print(f"\n{'='*70}")
            print(f"🔧 ROBUST FEDERATED AGGREGATION")
            print(f"{'='*70}")
            print(f"Pending updates: {len(self.pending_updates)}")
            
            # --- NEW: Create model savepoint for rollback ---
            ...
            # --- NEW: Calculate reputation-based weights ---
            weights_per_update = self._calculate_reputation_weights(self.pending_updates)
            
            # --- NEW: Trimmed Mean Aggregation ---
            aggregated_deltas = self._trimmed_mean_aggregation(
                self.pending_updates, 
                weights_per_update,
                trim_frac=TRIM_FRAC
            )
            
            # --- (C) NEW: Add Differential Privacy Noise ---
            aggregated_deltas = self._add_dp_noise(
                aggregated_deltas, 
                len(self.pending_updates),
                CLIP_BOUND
            )
            
            # --- NEW: Apply deltas to global model ---
            new_global_weights = {}
            for key in self.global_model['weights'].keys():
                old_value = self.global_model['weights'][key]
                delta = aggregated_deltas.get(key, 0.0)
                new_value = old_value + delta
                new_global_weights[key] = float(new_value)
            
            # --- NEW: Aggregate baseline statistics ---
            aggregated_baselines = self._aggregate_baselines_robust(self.pending_updates)
            new_global_weights.update(aggregated_baselines)
            
            # --- NEW: Calculate convergence ---
            convergence_delta = self._calculate_convergence(new_global_weights)
            
            # --- (D) NEW: Server-side Validation ---
            validation_metrics = self._validate_global_model(new_global_weights)
            
            # --- NEW: Rollback decision ---
            should_rollback = self._should_rollback(validation_metrics)
            
            if should_rollback:
                print(f"⛔ ROLLBACK: Validation AUC dropped significantly")
            ...
            # --- NEW: Update aggregation metadata ---
            ...
            # Update last validation metrics
            ...
            print(f"  Avg delta norm: {avg_norm:.4f}")
            print(f"  Quarantined clients: {quarantined}")
            print(f"  DP noise scale: {self.dp_noise_scale}")
            
            # Show significant weight changes
            for key in ['network_threshold', 'network_sensitivity', 'process_threshold']:
                if key in new_global_weights and key in old_weights:
                    old_val = old_weights[key]
                    new_val = new_global_weights[key]
                    if abs(old_val - new_val) > 0.01:
                        print(f"  {key}: {old_val:.3f} â†’ {new_val:.3f}")
...
class DatabaseManager:
    ...
    def analyze_and_alert(self, client_id, telemetry_data):
        """Analyze telemetry and generate DETAILED server-side alerts"""
        cursor = self.connection.cursor(cursor_factory=RealDictCursor)
        
        try:
            # ✅ PRIORITY 1: ML-DETECTED ANOMALIES with FULL DETAILS
            ml_alerts = telemetry_data.get('ml_anomaly_alerts', [])
            
            if ml_alerts:
                print(f"\n[SERVER] Processing {len(ml_alerts)} ML anomalies from {client_id[:12]}")
                
                for ml_alert in ml_alerts:
                    # ✅ FIX: Extract detection details
                    severity = ml_alert.get('severity', 'medium')
                    explanation = ml_alert.get('explanation', 'No details')
                    contributing_features = ml_alert.get('contributing_features', [])
                    iso_score = ml_alert.get('iso_score')
                    ae_error = ml_alert.get('ae_recon_error')
                    
                    # ✅ FIX: Build detailed description
                    description_parts = [explanation]
                    
                    if contributing_features:
                        features_str = ', '.join(contributing_features[:5])
                        description_parts.append(f"Contributing Features: {features_str}")
                    
                    if iso_score is not None:
                        description_parts.append(f"Isolation Forest Score: {iso_score:.4f}")
                    
                    if ae_error is not None:
                        description_parts.append(f"Autoencoder Error: {ae_error:.4f}")
                    
                    detailed_description = " | ".join(description_parts)
                    
                    # ✅ FIX: Calculate risk score from ML scores
                    risk_score = 5.0
                    if iso_score is not None:
                        # Map ISO score (-1 to 0) to risk (0-10)
                        risk_score = max(abs(iso_score) * 15, 5.0)
                    elif ae_error is not None:
                        risk_score = min(ae_error * 100, 10.0)
                    
                    # Only create alert for significant detections
                    if risk_score > 5.0 or severity == 'high':
                        self.create_alert(
                            client_id, 
                            'ml_ensemble_anomaly', 
                            severity,
                            f"ML Anomaly: {severity.upper()} - {contributing_features[0] if contributing_features else 'Multiple Features'}",
                            detailed_description,
                            risk_score
                        )
            
            # ✅ PRIORITY 2: DETAILED AUTO-ALERTS FROM HIGH-RISK TELEMETRY
            
            # Network alerts with DETAILS
            if 'network' in telemetry_data:
                net_details = telemetry_data['network'].get('details', [])
                
                for net_event in net_details:
                    risk_score = net_event.get('risk_score', 0)
                    detection_method = net_event.get('detection_method', 'unknown')
                    
                    # ✅ FIX: Only alert on ML detections OR very high risk
                    if (risk_score > 7.0 and detection_method == 'ml') or risk_score > 8.5:
                        dst_ip = net_event.get('dst_ip', 'unknown')
                        dst_port = net_event.get('dst_port', 0)
                        protocol = net_event.get('protocol', 'unknown')
                        conn_count = net_event.get('connection_count', 1)
                        threat_indicators = net_event.get('threat_indicators', [])
                        
                        # ✅ FIX: Build detailed description
                        description = (
                            f"Connection: {dst_ip}:{dst_port} ({protocol}) | "
                            f"Frequency: {conn_count} connections | "
                            f"Detection: {detection_method.upper()} | "
                            f"Indicators: {', '.join(threat_indicators[:3])}"
                        )
                        ...
                        self.create_alert(
                            ...
                            'network',
                            severity,
                            f'Suspicious Network Connection - {detection_method.upper()}',
                            description,
                            risk_score
                        )
            
            # Process alerts with DETAILS
            if 'processes' in telemetry_data:
                high_risk_procs = telemetry_data['processes'].get('high_risk', [])
                
                for proc_event in high_risk_procs:
                    risk_score = proc_event.get('risk_score', 0)
                    detection_method = proc_event.get('detection_method', 'unknown')
                    
                    if risk_score > 7.0:
                        proc_name = proc_event.get('process_name', 'unknown')
                        pid = proc_event.get('pid', 0)
                        parent_name = proc_event.get('parent_name', 'unknown')
                        exe_path = proc_event.get('executable_path', 'unknown')
                        exe_hash = proc_event.get('executable_hash', 'N/A')
                        threat_indicators = proc_event.get('threat_indicators', [])
                        ...
                        description = (
                            ...
                            f"Path: {exe_path[:60]}... | "
                            f"Hash: {exe_hash[:16] if exe_hash != 'N/A' else 'N/A'}... | "
                            f"Detection: {detection_method.upper()} | "
                            f"Indicators: {', '.join(threat_indicators[:3])}"
                        )
            ...
            # Filesystem alerts with DETAILS
            if 'filesystem' in telemetry_data:
                fs_events = telemetry_data.get('filesystem', [])
                
                for fs_event in fs_events:
                    risk_score = fs_event.get('risk_score', 0)
                    detection_method = fs_event.get('detection_method', 'unknown')
                    
                    if risk_score > 7.0:
                        file_name = fs_event.get('file_name', 'unknown')
                        file_path = fs_event.get('file_path', 'unknown')
                        file_ext = fs_event.get('file_extension', 'unknown')
                        file_hash = fs_event.get('file_hash', 'N/A')
                        threat_indicators = fs_event.get('threat_indicators', [])
                        
                        # ✅ FIX: Build detailed description
                        description = (
                            f"File: {file_name} ({file_ext}) | "
                            f"Path: {file_path[:60]}... | "
                            f"Hash: {file_hash[:16] if file_hash != 'N/A' else 'N/A'}... | "
                            f"Detection: {detection_method.upper()} | "
                            f"Indicators: {', '.join(threat_indicators[:3])}"
                        )
    ...
    def insert_process_data(self, client_id, process_records):
        ...
        for record in high_risk_list:
            try:
                ...
                # ✅ DEBUG: Log hash storage
                if exe_hash:
                    print(f"  [DB] Storing hash for {record.get('process_name')}: {exe_hash[:16]}...")
    ...
    def insert_filesystem_data(self, client_id, fs_records):
        ...
        for record in records_list:
            try:
                # CRITICAL: Convert all numpy types BEFORE any database operations
                record = convert_numpy_types(record)
                ...
                file_hash_value = str(record.get('file_hash')) if record.get('file_hash') else None
                    ...
                    # ✅ DEBUG: Log hash storage
                if file_hash_value:
                    print(f"  [DB] Storing hash for {record.get('file_name')}: {file_hash_value[:16]}...")
                        
                # Explicitly convert each value to native Python types
                cursor.execute("""
                ...
                """, (
                    ...
                    str(record.get('file_hash')) if record.get('file_hash') else None,
                    record.get('modification_time'),
                    str(record.get('directory')) if record.get('directory') else None,
                    bool(record.get('is_suspicious', False)),  # Explicit bool
                    float(record.get('risk_score', 0)),        # Explicit float
                    list(record.get('threat_indicators', []))  # Explicit list
                ))
            except Exception as e:
    ...
    def handle_client(self, client_socket, address):
        ...
        try:
            ...
            if data['type'] == 'registration':
                ...
                # ✅ FIX: Verify client exists, auto-register if missing
                ...
                if 'anomaly_alerts' in data and data['anomaly_alerts']:
                    ...
                    for anomaly_alert in data['anomaly_alerts']:
                        try:
                            ...
                            risk_score = 5.0
                            if iso_score is not None:
                                risk_score = abs(iso_score) * 10  # Convert to 0-10 scale
                            elif ae_error is not None:
                                risk_score = min(ae_error * 100, 10.0)
                            ...
                            """, (
                                ...
                                'ml_ensemble_anomaly',
                                severity_db,
                                f"ML Ensemble Anomaly Detection: {severity.upper()}",
                                anomaly_alert.get('explanation', 'No explanation available'),
                                'ml_detection',
                                float(risk_score)
                            ))
                            
                            print(f"  [SERVER] Stored client anomaly alert: {severity_db} (score={risk_score:.1f})")
                ...
                try:
                    ...
                    if 'user_activity' in data:
                        ...
                        self.db_manager.analyze_user_activity_anomalies(client_id, data['user_activity'])
...
class MainWindow(QMainWindow):
    ...
    def refresh_fl_tab(self):
        ...
        self.fl_metadata_label.setText(
            ...
            f"  Rollback: {'YES âš ï¸' if agg_metadata['rollback_occurred'] else 'No'}"
        )
