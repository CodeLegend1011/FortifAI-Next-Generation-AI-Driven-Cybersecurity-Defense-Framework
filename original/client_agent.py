...
class EnhancedAnomalyDetector:
    ...
    def _generate_alert(self, feature_vector, feature_names, anomaly_info):
        ...
        # âœ… NEW: PLAIN ENGLISH EXPLANATIONS
...
class ClientGUI:
    ...
    def refresh_anomalies(self):
        """Refresh anomaly list from agent - FIXED ISO SCORE EXTRACTION"""
        try:
            ...
            if not anomalies:
                self.anomaly_tree.insert('', 'end', text='0', values=('No anomalies detected', '', '', '', ''), tags=('normal',))
                return
            ...
            for i, alert in enumerate(anomalies):
                ...
                # ✅ CRITICAL FIX: Multi-level ISO score extraction
                iso_score = "N/A"
                
                # Priority 1: Model contributions raw_score
                if "isolation_forest" in model_contrib:
                    iso_raw = model_contrib["isolation_forest"].get("raw_score")
                    if iso_raw is not None:
                        iso_score = f"{iso_raw:.4f}"
                
                # Priority 2: Model contributions score
                if iso_score == "N/A" and "isolation_forest" in model_contrib:
                ...
                # Priority 3: Alert-level iso_score
                if iso_score == "N/A":
                    alert_iso = alert.get('iso_score')
                    if alert_iso is not None:
                        iso_score = f"{alert_iso:.4f}"
                
                # Priority 4: Calculate from ensemble_score if available
                if iso_score == "N/A":
                    ensemble = alert.get('ensemble_score', 0)
                    if ensemble > 6.0:
                        # Approximate: high ensemble usually means ISO contributed
                        iso_score = f"{ensemble/10:.2f}"

                # AE RECON ERROR - existing logic preserved
                ae_error = "N/A"
                if "autoencoder" in model_contrib:
                    ae_recon = model_contrib["autoencoder"].get("recon_error")
                    if ae_recon is not None:
                        ...
                        ae_contrib_score = model_contrib["autoencoder"].get("score")
                        if ae_contrib_score:
                            ae_error = f"{ae_contrib_score:.2f}"
                ...
                self.anomaly_tree.insert(
                    ...
                    values=(
                        ...
                        iso_score,  # ✅ NOW CORRECTLY POPULATED
                        ae_error,
                        features
                    ),
                    tags=(severity,)
